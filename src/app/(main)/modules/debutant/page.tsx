...
export const metadata = { title: 'Niveau Débutant — KaliAcademy' };

const level = moduleLevels.find(l => l.level === 'Débutant')!;
      ...
      <div className="container mx-auto px-4 py-16 max-w-3xl">
        ...
        <div className="mb-10">
          <div className="flex items-center gap-2 mb-3">
            <span className="h-2 w-2 rounded-full bg-[#5EFF8A]" />
            <span className="font-orbitron text-xs font-bold tracking-widest text-[#5EFF8A]">NIVEAU DÉBUTANT</span>
          </div>
          <h1 className="font-orbitron text-3xl sm:text-4xl font-black text-white mb-2">Fondations</h1>
          <p className="font-exo-2 text-sm text-gray-400 max-w-lg">
            Aucun prérequis nécessaire. Ces 4 modules posent les bases indispensables avant de progresser vers les niveaux suivants.
          </p>
        </div>
          ...
          {level.modules.map((mod, i) => (
            mod.available ? (
              <Link
                key={mod.id}
                href={mod.href}
                className="flex items-center justify-between rounded-xl border border-white/10 bg-white/3 px-5 py-4 hover:border-[#5EFF8A]/40 transition-all group"
              >
                <div className="flex items-center gap-4 min-w-0">
                  <span className="font-orbitron text-xs font-bold text-[#5EFF8A] shrink-0">{mod.number}</span>
        ...
        {/* Suivant */}
        <div className="mt-10 pt-8 border-t border-white/8 flex justify-end">
          <Link href="/modules/intermediaire" className="inline-flex items-center gap-2 font-exo-2 text-sm text-gray-500 hover:text-white transition-colors">
            Niveau Intermédiaire <ChevronRight className="h-4 w-4" />
          </Link>
        </div>
      </div>
    </div>
