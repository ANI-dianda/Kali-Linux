...
export const metadata = { title: 'Niveau Avancé — KaliAcademy' };

const level = moduleLevels.find(l => l.level === 'Avancé')!;
      ...
      <div className="container mx-auto px-4 py-16 max-w-3xl">
        ...
        <div className="mb-10">
          <div className="flex items-center gap-2 mb-3">
            ...
            <span className="font-orbitron text-xs font-bold tracking-widest text-red-400">NIVEAU AVANCÉ</span>
          </div>
          <h1 className="font-orbitron text-3xl sm:text-4xl font-black text-white mb-2">Expert & Professionnel</h1>
          <p className="font-exo-2 text-sm text-gray-400 max-w-lg">
            Prérequis : niveau Intermédiaire complet. Ces 6 modules couvrent les compétences recherchées par les entreprises.
          </p>
        </div>
          ...
          {level.modules.map((mod) => (
            mod.available ? (
              <Link
                key={mod.id}
                href={mod.href}
                className="flex items-center justify-between rounded-xl border border-white/10 bg-white/3 px-5 py-4 hover:border-red-400/40 transition-all group"
              >
                <div className="flex items-center gap-4 min-w-0">
                  <span className="font-orbitron text-xs font-bold text-red-400 shrink-0">{mod.number}</span>
        ...
        <div className="mt-10 pt-8 border-t border-white/8 flex justify-start">
          <Link href="/modules/intermediaire" className="inline-flex items-center gap-2 font-exo-2 text-sm text-gray-500 hover:text-white transition-colors">
            <ChevronLeft className="h-4 w-4" /> Niveau Intermédiaire
          </Link>
        </div>
      </div>
    </div>
